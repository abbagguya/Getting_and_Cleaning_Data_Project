...
# convert to tbl_df and isolate mean data
stat.data <- tbl_df(stat.data)
is.mean.col <- grep("mean", colnames(stat.data), fixed = TRUE)
is.std.col <- grep("std", colnames(stat.data), fixed = TRUE)
mean.data <- select(stat.data, c(1, 2, is.mean.col, is.std.col))

require(tidyr)

# subject, activity, measurement, mean are variables
gathered.mean.data <- gather(mean.data, measurement, mean, -(subject:activity))

# separate into measurement classification
tidy.mean.data <- separate(gathered.mean.data, measurement,
                           c("domain", "sensed", "direction", "stat"))

# group by subject, activity, domain, sensed, direction, stat
grouped.tidy.mean.data <-
    group_by(tidy.mean.data, subject, activity, domain, sensed, direction, stat)
...
mean.summary <- grouped.tidy.mean.data %>%
    ...
    arrange(subject, activity, desc(domain), sensed, direction, stat)

# output to a file
write.table(mean.summary, "UCI_HAR_mean.txt", sep = '\t', row.names = FALSE)
