...
mean.data <-
    select(stat.data, c(1, 2, grep("mean", colnames(stat.data), fixed = TRUE)))

# only mean data here -- remove "_mean" (5 characters)
substr(colnames(mean.data), 1, nchar(colnames(mean.data)) - 5)
...
# this is a strange mean that averages over time/frequency domains,
# sensed, direction
# tidy data on which filter function can be used to calcuate any grouped mean
